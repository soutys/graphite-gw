...
local function echo(status, message)
    ngx.status = status
    ngx.say(message)
    ngx.log(ngx.ERR, message .. ' (subject DN: ' .. tostring(ngx.var.ssl_client_s_dn) .. ')')
    ngx.exit(status)
end
...
if not ngx.var.ssl_client_s_dn then
    echo(530, 'Cannot get subject DN')
end
...
local function get_graphite_out()
    ...
    if not graphite_out then
        graphite_out = ngx.req.get_body_data()
    end
...
local function send_to_graphite(graphite_address, graphite_port, graphite_out)
    ...
    if not ok then
        echo(532, 'Failed to connect to graphite: ' .. graphite_address .. ':' .. graphite_port)
    end

    local bytes, err = sock:send(graphite_out)
    if not bytes then
        echo(533, 'Cannot send to graphite: ' .. tostring(err))
    end
...
local function match_graphite_out(acls, graphite_out)
    for cert_regex, acl in pairs(acls) do
        if match_lines(client_name, cert_regex) then
            for _, metrics_regex in pairs(acls[cert_regex]) do
